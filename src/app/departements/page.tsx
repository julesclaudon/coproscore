...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://coproscore.fr/departements",
  },
};
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {depts.map((d) => {
              ...
              return (
                <Link
                  key={d.code_dept}
                  href={`/departement/${makeDeptSlug(d.nom_dept, d.code_dept)}`}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-slate-900 group-hover:text-teal-700">
                      {d.nom_dept}
                      <span className="ml-1.5 font-normal text-slate-400">({d.code_dept})</span>
                    </p>
                    <p className="mt-0.5 text-xs text-slate-400">
                      {Number(d.total).toLocaleString("fr-FR")} copro{Number(d.total) > 1 ? "s" : ""} ·{" "}
                      {Number(d.nb_communes).toLocaleString("fr-FR")} commune{Number(d.nb_communes) > 1 ? "s" : ""}
                    </p>
                  </div>
